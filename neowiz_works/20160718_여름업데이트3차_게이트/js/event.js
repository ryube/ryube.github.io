...
function layerBtn() {
	...
	function openAni(t){
		...
		$('.pop_dim').addClass('on').stop(true, false).animate({ opacity: .9}, 250, 'easeOutQuad',function(){});
		//docScrollBlock(true);
	}
...
$(function(){
	...
	// Scroll Event
	var gnbEl = $('.wrap_pm_gnb, .ev_01'),
		$fixBg = $('.fix_bg'),
		gnbOffSet = gnbEl.offset();

	ev3Offset = $('#ev_03').offset();
	
	$fixBg.css('top', gnbOffSet.top);
	if(gnbEl.length != 0){
		$(window).off().on('scroll',function(){
			...
			if(ev3Offset.top-400<wsTop){
				$fixBg.css('bottom', 165);
			} else {
				$fixBg.css('bottom', 0);
			}
