        ...
        setupPaging: function() {
          ...
          if (slider.pagingCount > 1) {
            for (var i = 0; i < slider.pagingCount; i++) {
              ...
              slider.controlNavScaffold.append('<li class="li'+i+'">' + item + '</li>');
              j++;
            }
          }
...
function replaceNode(template, replaceNode, text, defaultDigitCount) {
    var i, txtArr = text;

    for (i = txtArr.length; i < defaultDigitCount; i++) {
        txtArr = '0' + txtArr;
    }
    txtArr = txtArr.split('');
    replaceNode.text('');
    ...
    for (i = txtArr.length - 1; i >= 0; i--) {
        replaceNode.prepend(template.split('{$data}').join(txtArr[i]));
    }
}
////////////////////////////////////////////////////////////////////////////////
function reloadUi() {
	...
	layerBtn();
	pmGnb();
	var number	= $('.ev_01 .number');
	var numberCount= number.text();	
    replaceNode('<span class="num num_{$data}">{$data}</span>', number, numberCount, 3);
	$('#layer_party').flexslider({
		animation: "fade",
		slideshow: false,
		directionNav: true,
		controlNav: false,
