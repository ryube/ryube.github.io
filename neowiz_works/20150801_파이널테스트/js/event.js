...
function reloadUi() {
	...
	scrollMove();
	placeHoldBg();
	tabContents('.ev_01 .tab_1', '.ev_01 .tab_contents', 0)
	tabContents('.ev_01 .tab_2', '.ev_01 .tab2_contents', 0)
	finalBannerSlider();
	ytChange();
	$('.btn_mov:eq(0)').click();

	$('.tab_1 a').eq(0).click(function(){
		$('.btn_mov:eq(0)').click();
	});
	$('.tab_1 a').eq(1).click(function(){
		$('.promo_video').find('iframe, img').detach();
	});
};
...
$(function(){
	...
	if ($('.wrap').hasClass('betaopen')) {
		var r = Math.random();
		if(r>0.5){
			$('.ev_loop .bg2').addClass('s2');
		}
	};
	
		console.log('btnMt'+btnMt)
	...
	if(gnbEl.length != 0){
		$(window).bind('scroll',function(){
			...
			if (gWh > wsTop) {
				mRatio = wsTop/gWh*0.9;
				$mask1.css({border:mRatio*180+"px solid #101010"});
				$mask2.css({opacity:mRatio});
				console.log(btnMt-mRatio*10)
				$btnCustom.css({marginTop: btnMt+mRatio*80, opacity:1-wsTop/gWh});
			};
	...
	if(window.location.hash){
		...
		el = hashUrl.split('/').pop();
		if(el.indexOf('#') == '-1' ){el = '#'+el}
		if($(el).length > 0){
			var hashElOffset = $(el).offset();
			setTimeout(function(){
				$('html:not(:animated), body:not(:animated)').stop(true, false).animate({ scrollTop: hashElOffset.top}, 2000, 'easeInOutQuint');
			}, 500)
		}
	}
