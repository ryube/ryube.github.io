...
$(function(){
	...
	var gnbEl = $('.wrap_pm_gnb, .ev_loop'),
		...
		$tl = $('.title'),
		$btnCustom = $('.btn_cus'),
		tlMt = parseInt($tl.css('margin-bottom'), 10),
		btnMt = parseInt($btnCustom.css('margin-bottom'), 10),
		mRatio,
		gWh = $(window).height(),
	...
	$tl.delay(500).animate({opacity: 1, bottom: 0}, 1500, 'easeOutQuint',function(){
		$btnCustom.animate({opacity: 1, bottom: 50}, 500, 'easeOutQuint',function(){});
	});
	$mask1.delay(950).animate({height: 0}, 800, 'easeOutQuint',function(){});
	...
	if(gnbEl.length != 0){
		$(window).bind('scroll',function(){
			...
			$btnCustom.stop(true, false).css({marginBottom: btnMt-mRatio*250, opacity:1-wsTop/gWh});
			$tl.stop(true, false).css({marginBottom: tlMt+mRatio*500, opacity:1-wsTop/gWh*2});
		});
	}
