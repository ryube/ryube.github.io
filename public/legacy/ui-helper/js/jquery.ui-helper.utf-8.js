...
(function ($) {
  if ($.u.theme == "flat") {
    $.u.css = "/legacy/ui-helper/css/ui-helper.css"; /*  */
  } else if ($.u.theme == "mini") {
    $.u.css = "/legacy/ui-helper/css/ui-helper-minimal.css";
  } else {
    $.u.css = "/legacy/ui-helper/css/ui-helper-legacy.css";
  }
  $("<link>")
    .attr({
      type: "text/css",
  ...
  if ($.u.draggable == false) {
    $.u.dragstop = ' ondragstart="return false" onselectstart="return false"';
  } else {
    $.u.dragstop = "";
  }
  $("body").append('<div id="ui-helper"' + $.u.dragstop + "></div>");
  $("#ui-helper")
    .addClass("on")
    .append(
      '<div class="base"><div class="head"><h2>ui helper</h2><i class="icon-cancel-circled-outline close"><span>X</span></i></div><div class="body"></div><div class="foot"></div></div><div class="btn_panel"><i class="icon-plus-circled close" title="ui helper"><span>+</span></i></div>'
    );
  $.each($.u.helper, function (i) {
    $.u.html = "";
    $.each($.u.helper[i].items, function (ii) {
      if ($.u.helper[i].items[ii].checked) {
        $.u.checked = ' checked = "checked"';
        $.u.helper[i].items[ii].run();
      } else {
        $.u.checked = "";
      }
      if ($.u.helper[i].items[ii].input == "button") {
        $.u.button = " button";
      } else {
        $.u.button = "";
      }
      if ($.u.helper[i].items[ii].disabled) {
        $.u.disabled = ' disabled="disabled"';
      } else {
        $.u.disabled = "";
      }
      if ($.u.helper[i].items[ii].tooltip) {
        $.u.tooltip = ' title="' + $.u.helper[i].items[ii].tooltip + '"';
      } else {
        $.u.tooltip = "";
      }
      $.u.html +=
        "<li><h4>" +
        $.u.helper[i].items[ii].subject +
        "</h4><p>" +
        $.u.helper[i].items[ii].description +
        '</p><p class="switch' +
        $.u.button +
        '"><input class="radio" type="' +
        $.u.helper[i].items[ii].input +
        '" id="input' +
        i +
        "-" +
        ii +
    ...
    if ($.u.helper[i].tooltip) {
      $.u.tooltip = ' title="' + $.u.helper[i].tooltip + '"';
    } else {
      $.u.tooltip = "";
    }
    $("#ui-helper .body").append(
      '<h3 rel="' +
        $.u.fold +
        '"' +
        $.u.tooltip +
  ...
  if ($.u.animate) {
    ...
    $("#ui-helper").css("margin-left", $.u.marginLeft);
  }
  if ($.u.position == "absolute") {
    $("#ui-helper").css({
      position: $.u.position + " !important",
      top: $.u.top,
    });
  } else {
    $("#ui-helper").css({
  ...
  $('#ui-helper h3[rel="close"]').addClass("off").next("ul").hide();
  $('#ui-helper .switch:not(".button") input').after(function () {
    ...
    if ($(this).is(":checked")) {
      if ($(this).is(":disabled")) {
        return (
          "<a href='#' class='toggle checked disabled' rel='" +
          $(this).attr("rel") +
          "' title='" +
          $(this).attr("title") +
          "'" +
        ...
        return (
          "<a href='#' class='toggle checked' rel='" +
          $(this).attr("rel") +
          "' title='" +
          $(this).attr("title") +
          "'" +
      ...
      if ($(this).is(":disabled")) {
        return (
          "<a href='#' class='toggle disabled' rel='" +
          $(this).attr("rel") +
          "' title='" +
          $(this).attr("title") +
          "'" +
  ...
  $("#ui-helper .button input").after(function () {
    if ($(this).is(":disabled")) {
      return (
        "<a href='#' class='default' rel='" +
        $(this).attr("rel") +
        "' title='" +
        $(this).attr("title") +
        "' disabled='disabled'>" +
  ...
  $("#ui-helper .toggle").bind("click", function (e) {
    e.preventDefault();
    if (!$(this).hasClass("disabled")) {
      $.u.rel = $(this).attr("rel").split("/");
      $.u.input = $("#input" + $.u.rel[0] + "-" + $.u.rel[1]);
      $.u.type = $.u.input.attr("type");
      if ($.u.type == "radio") {
        $.u.radio = $(this).attr("name");
        $('#ui-helper .toggle[name="' + $.u.radio + '"]').each(function () {
          $.u.rel2 = $(this).attr("rel").split("/");
          $.u.input2 = $("#input" + $.u.rel2[0] + "-" + $.u.rel2[1]);
          $(this).removeClass("checked");
          $.u.input2.removeAttr("checked");
        });
        $.u.input.attr("checked", "true");
        $(this).addClass("checked");
      } else {
        if ($.u.input.is(":checked")) {
          $.u.input.removeAttr("checked");
        } else {
          $.u.input.attr("checked", "true");
        }
        $(this).toggleClass("checked");
      }
      $.u.helper[$.u.rel[0]].items[$.u.rel[1]].run();
    }
  });
  $("#ui-helper .default").bind("click", function (e) {
    e.preventDefault();
    $.u.rel = $(this).attr("rel").split("/");
    $.u.helper[$.u.rel[0]].items[$.u.rel[1]].run();
  });
  $("#ui-helper .base .head .close").click(function () {
    $("#ui-helper").removeClass("on");
  });
  $("#ui-helper h2").dblclick(function () {
    $("#ui-helper .base .head .close").trigger("click");
  });
  $("#ui-helper .btn_panel .close").click(function () {
    $("#ui-helper").addClass("on");
  });
  $("#ui-helper h3").click(function () {
  ...
  if ($.u.draggable == true) {
    ...
    $("#ui-helper").draggable($.u.axis);
    $("#ui-helper h2").css("cursor", "move");
  }
  if (($.u.theme == "flat" || $.u.theme == "mini") && $.u.color) {
    $("#ui-helper h2").css("background-color", $.u.color);
    $("#ui-helper .base .body h3").css("color", $.u.color);
    $(
      '<style type="text/css">#ui-helper .base .body li .switch a.checked,.ui-tooltip { background: ' +
        $.u.color +
        "; }#ui-helper .base .body li .switch a.default { background: " +
        $.u.color +
        "; }</style>"
    ).appendTo("head");
  }
